...
#' 'parallel' package for multicore processing. It will also show a progress bar
#' if the 'pbapply' package is installed.
#'
#' @param wave A Wave object or filename. Using filenames may save loading an entire large file into memory.
#' @param window.length The length of the analysis window (in samples).
#' @param window.overlap The overlap between successive windows (in samples).
#' @param bind.wave If TRUE and FUN returns wave objects, then these are combined into a single object
#' @param FUN FUN to be applied to windows.
#' @param ... Additional parameters to FUN
#' @param complete.windows If TRUE (default) the final window will not be processed unless it has a length equal to window.length.
...
windowing <- function(
  ...
  if (complete.windows) {
    ...
    n.windows <- floor(n.samples / (window.length - window.overlap))
  }
  starts <- c(
    1,
